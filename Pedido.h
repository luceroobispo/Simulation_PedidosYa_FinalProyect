...
public:
		...
		do {
			...
			for (int i = 0; i < pro.size(); ++i) {
				if (pro[i]->getnproducto() == nombre) {
					...
					cin >> cantp;
					lpedidos.push_back(Pedido(pro[i]->getnproducto(), pro[i]->getmarca(), (float)pro[i]->getprecio(), cantp, pro[i]->getcategoria()));
	...
	void DesordenarPedidos() { //RECURSIVIDAD
		...
		lpedidos.desordenar(v);
		MostrarPedidos();
	}

	void OrdenarPedidos() {
		function<bool(Pedido, Pedido)>  Concicion = [](Pedido p, Pedido p2) {return p.getprecio()*p.getcantp() < p2.getprecio() * p2.getcantp(); };
		lpedidos.mergeSort(lpedidos.getarr(), lpedidos.getpos()+1, Concicion);
		cout << "PEDIDOS ORDENADOS" << endl << endl;
		MostrarPedidos();
	}
